...
import {
  ...
  type StoreCollectionReference,
  type StoreDocumentReference,
  type StoreDocumentSnapshot,
  type StoreQuery,
...
class MemoryStore implements JobPostingStore {
  ...
  public collection(path: string): StoreCollectionReference {
    ...
    const makeQuery = (order: { field: string; direction: "asc" | "desc" } | null, limit: number | null): StoreQuery => ({
      ...
      get: async () => {
        let docs = [...documents.entries()]
          ...
          .map(([key, data]) => snapshot(key.slice(prefix.length), data));
        if (order) {
          docs.sort((a, b) => {
    ...
    const snapshot = (id: string, data: DocumentData | undefined): StoreDocumentSnapshot => ({
      id,
  ...
  public async runTransaction<T>(updateFunction: (transaction: StoreTransaction) => Promise<T>): Promise<T> {
    ...
    const transaction: StoreTransaction = {
      get: async (reference) => {
        const data = documents.get(keyOf(reference));
        return { id: reference.id, exists: data !== undefined, data: () => (data ? { ...data } : undefined) };
      },
